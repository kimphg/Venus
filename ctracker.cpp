...
//output:	m_Featlist[FEATNUM][5]		//global variable to store feature value
//			m_Featlabels[FEATNUM][10]	//global variable to store feature label
...
{
    POINT	ApexPoint[4]; //input track region points
    POINT	ApexPointHalf[4];
    ApexPoint[0].x = m_RectInit.left;
    ApexPoint[0].y = m_RectInit.top;
    ApexPoint[1].x = m_RectInit.left;
    ApexPoint[1].y = m_RectInit.bottom;
    ApexPoint[2].x = m_RectInit.right;
    ApexPoint[2].y = m_RectInit.bottom;
    ApexPoint[3].x = m_RectInit.right;
    ApexPoint[3].y = m_RectInit.top;

    for(int i=0; i<4; i++)
    {
        ApexPointHalf[i].x = ApexPoint[i].x/2;
        ApexPointHalf[i].y = ApexPoint[i].y/2;
    }

    m_RegionCurrent = CreatePolygonRgn(ApexPoint, 4, WINDING);
    m_RegionCurrentHalf = CreatePolygonRgn(ApexPointHalf, 4, WINDING);

    CreateRegionMask(m_RegionCurrent, gImgInitMask);
    CreateRegionMask(m_RegionCurrentHalf, gImgInitMaskHalf);
}

void CTracker::CreateRegionMask(HRGN inRegion, IplImage *outImgMask)
{
    RECT rgnBox;
    BOOL bResult;
    int	 row, col;
